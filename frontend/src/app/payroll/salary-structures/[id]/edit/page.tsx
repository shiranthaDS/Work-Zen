...
export default function EditSalaryStructurePage() {
  ...
  const fetchSalaryStructure = async () => {
    try {
      const response = await salaryStructureApi.get(params.id as string);
      ...
      setFormData({
        ...
        pension_rate: data.pension_rate || 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await salaryStructureApi.update(params.id as string, formData);
      toast.success('Salary structure updated successfully');
      router.push('/payroll/salary-structures');
    } catch (error: any) {
      toast.error(error.response?.data?.detail || 'Failed to update salary structure');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Edit Salary Structure</h1>
          <p className="text-gray-500">Update employee compensation structure</p>
        </div>
      </div>
